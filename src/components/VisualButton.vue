<template>
  ...
  <!-- 個別編集モード用コントロールパネル -->
  <div v-if="props.isSelected && !props.isLayoutMode" class="controls" @mousedown.stop>
    <label>
      テキスト:
      <input type="text" v-model="displayContent" />
    </label>
    <label>
      フォントサイズ: {{ fontSize }}px
...
// --- 双方向バインド用 refs ---
const displayContent = ref(state.value.content);
...
// --- watch で state に反映 ---
watch(displayContent, v => state.value.content          = v);
